    ...
    private HttpService() {
        final HttpLoggingInterceptor httpLoggingInterceptor = new HttpLoggingInterceptor(new HttpLoggingInterceptor.Logger() {
            ...
            public void log(String message) {
                if ((message.contains(Constant.PAY_ALIPAY)||message.contains(Constant.PAY_WEIXIN)) && message.contains("\"status\":-22")) {
                    EventBus.getDefault().post(new EventMessage(Constant.ERROR, message));
                }
            }
        });
        httpLoggingInterceptor.setLevel(HttpLoggingInterceptor.Level.BODY);
        OkHttpClient okHttpClient = new OkHttpClient.Builder()
                .addInterceptor(httpLoggingInterceptor) //日志打印拦截器
